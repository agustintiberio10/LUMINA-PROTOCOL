...
/// @title EchidnaFlashETHShield1h — Sprint T-30a Phase F
/// @notice Property scaffold for the ETH 1h / 4% flash shield.
...
contract MockOracleE {
    int256 public answer = 3_000e8; // ETH seed
...
contract EchidnaFlashETHShield1h {
    ...
    uint16 internal constant TRIGGER_DROP_BPS = 400;
    ...
    constructor() {
        ...
        shield = FlashETHShield1h(
            address(
                new ERC1967Proxy(
                    address(new FlashETHShield1h()),
                    abi.encodeCall(FlashETHShield1h.initialize, (ROUTER, address(oracle), address(sequencer)))
                )
            )
        );
    }

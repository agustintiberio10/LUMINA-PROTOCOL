...
/// @title FlashBTCShield48h unit tests (Sprint T-30a Phase F)
/// @notice BTC 48h / 10% trigger variant.
...
contract FlashBTCShield48hTest is Test {
    ...
    uint16 constant TRIGGER_DROP_BPS = 1000; // 10%
    ...
    function testVerify_3ConfirmationsRequired() public {
        ...
        vm.warp(T0 + 60);
        int256 dropped = (STRIKE * 8900) / 10_000; // 11% drop, above 10%
        oracle.setAnswer(dropped, T0 + 60);
        vm.prank(router);

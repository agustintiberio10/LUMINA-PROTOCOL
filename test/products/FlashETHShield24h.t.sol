...
/// @title FlashETHShield24h unit tests (Sprint T-30a Phase F)
/// @notice ETH 24h / 8.5% trigger variant.
...
contract FlashETHShield24hTest is Test {
    ...
    uint16 constant TRIGGER_DROP_BPS = 850; // 8.5%
    ...
    function testVerify_3ConfirmationsRequired() public {
        ...
        vm.warp(T0 + 60);
        int256 dropped = (STRIKE * 9000) / 10_000; // 10% drop, above 8.5%
        oracle.setAnswer(dropped, T0 + 60);
        vm.prank(router);

...
contract FlashBTCShield4h is BaseShield {
    ...
    uint32 public constant MIN_DURATION = 14400; // 4h
    uint32 public constant MAX_DURATION = 14400; // 4h (fixed)
    uint32 public constant WAITING_PERIOD = 0;

    uint256 public constant DEDUCTIBLE_BPS = 2000; // 20% deductible → 80% max payout
    uint256 public constant TRIGGER_DROP_BPS = 800; // 8% drop
    ...
    struct BSSData {
        bytes32 asset; // "BTC"
        int256 strikePrice;
        int256 triggerPrice; // strikePrice × (100 - 8) / 100
    }
    ...
    function _doCreatePolicy(uint256 policyId, CreatePolicyParams calldata params) internal override {
        ...
        // Trigger: strikePrice × (100 - 8) / 100 = strikePrice × 92 / 100
        ...
        returns (PayoutResult memory result)
    {
        ...
        result = PayoutResult({
            triggered: true, payoutAmount: cp.maxPayout, recipient: cp.insuredAgent, reason: "FLASHBTC4H_DROP8"
        });
    }

...
/// @notice Insurance against a crash/spike of BTC spot price within 48 hours on BTC.
/// @dev Covered asset: BTC. Premium is always paid in USDC.
///      Trigger condition: Crash/spike of BTC spot price within 48 hours (price drops >15%
...
contract FlashBTCShield48h is BaseShield {
    ...
    uint32 public constant MIN_DURATION = 172800; // 1h
    uint32 public constant MAX_DURATION = 172800; // 1h (fixed)
    uint32 public constant WAITING_PERIOD = 0;

    uint256 public constant DEDUCTIBLE_BPS = 2000; // 20% deductible → 80% max payout
    uint256 public constant TRIGGER_DROP_BPS = 1500; // 15% drop
    ...
    function _doCreatePolicy(uint256 policyId, CreatePolicyParams calldata params) internal override {
        ...
        // Trigger: strikePrice × (100 - 15) / 100
        ...
        returns (PayoutResult memory result)
    {
        ...
        result = PayoutResult({
            triggered: true, payoutAmount: cp.maxPayout, recipient: cp.insuredAgent, reason: "FLASHBTC48_DROP15"
        });
    }

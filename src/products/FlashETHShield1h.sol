...
/// @notice Insurance against a crash/spike of ETH spot price within 1 hour on ETH.
/// @dev Covered asset: ETH. Premium is always paid in USDC.
///      Trigger condition: Crash/spike of ETH spot price within 1 hour (price drops >7%
///      from the exact price at policy issuance block).
///      Duration: 1 hour (fixed; no waiting period).
///      Payout: Binary — 80% of coverage (20% deductible).
///
/// PRODUCT: FLASHETH1H-001
/// RISK TYPE: VOLATILE
///
...
contract FlashETHShield1h is BaseShield {
    ...
    function _doCreatePolicy(uint256 policyId, CreatePolicyParams calldata params) internal override {
        ...
        // Trigger: strikePrice × (100 - 7) / 100 = strikePrice × 93 / 100
        ...
        returns (PayoutResult memory result)
    {
        ...
        result = PayoutResult({
            triggered: true, payoutAmount: cp.maxPayout, recipient: cp.insuredAgent, reason: "FLASHETH1H_DROP7"
        });
    }

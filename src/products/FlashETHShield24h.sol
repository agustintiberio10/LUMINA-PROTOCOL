 ...
 * DURATION: Fixed 24h. No waiting period (instant coverage).
...
contract FlashETHShield24h is BaseShield {
    ...
    uint32 public constant MIN_DURATION = 86400; // 24h
    uint32 public constant MAX_DURATION = 86400; // 24h (fixed)
    ...
    uint256 public constant TRIGGER_DROP_BPS = 2000; // 20% drop
    ...
    struct BSSData {
        ...
        int256 triggerPrice; // strikePrice × (100 - 20) / 100
    }
    ...
    function _doCreatePolicy(uint256 policyId, CreatePolicyParams calldata params) internal override {
        ...
        // Calculate trigger price: strikePrice × (100 - 20) / 100 = strikePrice × 80 / 100
        ...
        returns (PayoutResult memory result)
    {
        ...
        result = PayoutResult({
            triggered: true, payoutAmount: cp.maxPayout, recipient: cp.insuredAgent, reason: "FLASHETH24_CRASH"
        });
    }

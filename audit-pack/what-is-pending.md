...
- **2026-05-26 (Sprint UUPS Upgrade Manual Review On-Chain)**: ✅ **Phase E del Sprint Fix Manual Review CERRADA on-chain** — 8 UUPS upgrades broadcast en Base Sepolia con las addresses canónicas V5.4 (PR #160). Dry-run limpio, storage append-only verificado, selectors + smoke tests verdes (purchase policyId=2 tx `0x50530d1a…`; executeOffer revert AccessControl; getLuminaPrice 0.036e18 con gate MR-H01 dormant porque `pool()==0x0`). New impls: CapacityOracle `0x8991Ae04`, CoverRouter `0x161f9d80`, PolicyManager `0x63586DA3`, BondVault `0x3D5f4329`, BuybackEngine `0x93b05984`, TWAPBurner `0xAe327b5c`, ClaimBond `0x8Fd6b84a`, AdaptiveFeeDist `0x9E2859AB`. Gas 18.2M (~0.0002 ETH). Sprint doc `audit-pack/sprints/2026-05-26-sprint-upgrade-manual-review-onchain.md`. PR `feat/upgrade-manual-review-onchain`. **Gaps que siguen abiertos**: ShieldKeeper (no wired), CEXLiquidityReserve (dormant → MR-M03 reserve-side NO on-chain), 2 DEX adapters MR-M06 (redeploy pendiente), `setFreshnessParams` explícito al wirear pool mainnet.
- **2026-05-25 (Sprint Canonical Manifest V5.4 + Dry-run Sprint UUPS Upgrade)**: inventario on-chain read-only generó `audit-pack/manifests/V5.4-canonical-deployed.{json,md}` (direcciones live derivadas de los root contracts). **Gaps confirmados que bloquean el broadcast del Sprint UUPS Upgrade (NO ejecutado)**: (1) **ShieldKeeper NO wired** — `adapter.keeper()==0x0` en los 6 adapters V5.4; settlement corre vía relayer (`onlyKeeperOrRelayer`) — desplegar+wirear o aceptar relayer-only. (2) **CEXLiquidityReserve dormant** — `bondVault.cexReserve()==0x0`; MR-M03 reserve-side no puede ir on-chain hasta desplegarla (ver `CEX-RESERVE-DEFERRED`). (3) **Manifests del repo con addresses STALE V5.0** (CapacityOracle/SolvencyOracle/MaintenanceReserve/ShieldKeeper) → el dry-run de `UpgradeAll-FixManualReview.s.sol` revirtió usando `0xAf99` (V5.0). Pre-broadcast: usar el manifest canónico, corregir Phase-F selectors (los del spec no existen — ver report), MR-H02 es off-chain (no upgrade), MR-M06 DEX adapters son redeploy no upgrade. PR `feat/canonical-manifest-v54`.

...
- **2026-05-26 (Sprint 7.6 Operational Security Audit)**: auditoría operacional read-only (procesos/runbooks/OpSec/IR/monitoring/governance/legal — NO código). **Score 6.0/10 ⚠️ NEEDS HARDENING.** Docs operacionales sorprendentemente maduras (suite completa de runbooks, IR, governance, monitoring, legal, key-rotation, multisig, training) PERO brecha documentado-vs-ejecutado: 1 CRIT (OP-001 key-rotation plan NO ejecutado; priv-key founder en plaintext, reusada en broadcasts, no rotada), 4 HIGH (OP-002 main sin branch protection + sin CODEOWNERS; OP-003 single EOA dueño de los 8 proxies sin multisig/timelock; OP-004 IR asume equipo inexistente = team-of-one SPOF; OP-007 runbooks nunca testeados), 7 MED, 3 LOW, 1 INFO. Fase 5 testnet: SÍ; mainnet: NO hasta cerrar keys+multisig+SDLC+on-call (4-6 semanas). Report `audit-pack/audits/2026-05-26-operational-audit-v53.md`. PR `feat/audit-7-6-operational-v1`.
- **2026-05-25 (Sprint Canonical Manifest V5.4)**: inventario on-chain read-only — **derivadas TODAS las direcciones live V5.3/V5.4 desde los contratos raíz** (`cast call` getters + ERC1967 impl slot), no del repo. Hallazgo clave: los manifests del repo tienen **direcciones STALE V5.0** para CapacityOracle (repo `0xAf99…` ≠ live `0xd52aef11…6545`), SolvencyOracle (live `0x23f77D17…`), MaintenanceReserve (live `0xD78A72e9…`), ShieldKeeper. Esto explica el revert del dry-run del Sprint UUPS Upgrade (usaba `0xAf99` deprecated). **Gaps confirmados**: ShieldKeeper NO wired (`adapter.keeper()==0x0`, settlement vía relayer), CEXLiquidityReserve dormant (`bondVault.cexReserve()==0x0`). Sin contratos rotos — todos los proxies live con impl no-cero + wiring consistente. Manifest canónico en `audit-pack/manifests/V5.4-canonical-deployed.{json,md}`. PR `feat/canonical-manifest-v54`. NO se modificó nada on-chain (solo lectura).

...
|---|---|---|
| **C-1** | ✅ Fixed | `PolicyManagerV2.sol` — `IShieldV2.CreatePolicyParams` now uses `bytes extraData` (matches `IShield`). `recordPolicy` passes `extraData: ""`. |
| **H-1** | ✅ Documented | NatSpec header on all 5 V2 shields (`src/v2/products/*.sol`) states router_ must be PolicyManagerV2. Deploy order captured in `script/V2-DEPLOY-ORDER.md`. |
| **H-2** | ✅ Fixed | `TWAPBurner.sol` — new `capacityOracle` state + `setCapacityOracle()` setter. `executeBurn` derives `amountOutMin` from oracle price × `(1 − maxSlippageBps)` via try/catch; falls back to 0 if oracle unset. |
| **H-3** | ✅ Fixed | `BondVault.sol` — new `BREAKER_COOLDOWN = 1 hours` + `lastBreakerTriggerTime` state. `resetCircuitBreaker` requires `block.timestamp ≥ lastBreakerTriggerTime + BREAKER_COOLDOWN`. |
| **M-1** | ✅ Fixed | `PolicyManagerV2.sol` — `recordPolicy` increments `totalPolicies/activePolicies` before calling `shield.createPolicy`. `triggerPayout` sets `pr.triggered/activePolicies/totalTriggers/totalBondsIssuedUSD` before external `verifyAndCalculate` and `issueBond`. |
| **M-3** | ✅ Fixed | `TWAPBurner.sol` — imports SafeERC20, uses `IERC20(token).safeTransfer(...)` in `recoverToken`. |
| **M-4** | ✅ Fixed | `CoverRouterV2.sol` — `usdc.forceApprove(twapBurner, premium)` in `_purchase` (SafeERC20 was already imported). |
| **M-6** | ✅ Fixed | `PolicyManagerV2.sol` — `require(payoutUSD > 0, "Payout too small for bond issuance")` before `bondVault.issueBond`. |
| **M-7** | ✅ Fixed | `CapacityOracle.sol` — `luminaToken` and `usdcToken` marked `immutable`. |
| **L-9** | ✅ Fixed | `PolicyManagerV2.sol` — `bondVault` marked `immutable`. |
| **L-10** | ✅ Documented | `LuminaTokenV2.sol` — NatSpec warning about DEFAULT_ADMIN_ROLE renounce consequence added to the contract-level doc. |
| **configureProduct duration=0** | ✅ Fixed | `CoverRouterV2.sol` — `require(_durationSeconds > 0, "Duration must be > 0")` at top of `configureProduct`. |

### Not yet fixed (deferred — require additional work)
- **H-2 (keeper refinement)**: oracle-priced `amountOutMin` is in place but uses the same TWAP source as BondVault, so a single oracle failure/compromise affects both. Production should consider a second independent price source (Chainlink LUMINA feed when available).
- **M-2 (TWAPBurner reentrancy-balance)**: cosmetic; `nonReentrant` already covers it. No code change needed.
- **M-5 (relayer trust boundary)**: documented in this report; no on-chain fix (by design).
- **L-1 through L-8**: acknowledged; see per-item rationale above. None are blocking.
